/**
 * Framework reference generators — one per answer type. Kept registered because the framework's own
 * unit tests (tests/unit/problems/components.test.tsx, checkpoints.test.ts) and docs/problem-authoring.md
 * cite these ids. The Prologue's own drills live in shakedown.ts, heat-budget.ts and tasking.ts; no
 * Act 0 module or checkpoint uses these five.
 *
