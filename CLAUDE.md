...
- **Spelled-out numbers in dialogue are allowed when a test pins the value.** "Four hundred and twelve hulls" reads better in speech than `{fmtInt(manifest.length)}`, and the rule exists to stop numbers drifting from the data, not to ban English. So a word-form number may appear in prose or dialogue **only if** the Act's `data.test.ts` asserts that exact value, so a dataset change fails the suite instead of silently making the prose a lie. The test: **does this number exist in `data.ts`?** If yes, it is a dataset value — compute it, or pin it. If it is narrative colour invented for the scene and living nowhere else (a pulse rate, a compartment temperature, how many frames aft somebody dropped a panel), words are fine and no test is owed. Digits in prose, every briefing figure, every beat answer and every worked-solution number are still computed — no exceptions.
...
17. **The corrective reframe: "it's not X, it's Y."** A negation followed by the real answer, used as the default way to make a point. "The code isn't a measurement, it's a finding." "That is not a failure of the method; it is the method telling you the truth." It is a rhetorical move rather than a sentence, and at scale it becomes a drone — there were **39 instances in Acts II–VI** when this was written. Say the positive thing: "The code is a finding." Variants to avoid: "not because A, but because B", "The problem is not A. The problem is B", "X is not A; it is B". **One narrow exception:** where the negation is the misconception being taught and the reader genuinely holds it (density versus probability, mutually exclusive versus independent). Even then, prefer stating the truth first and the error second, and keep it to once per module.
18. **The epigram, and the character who reads the protagonist's soul.** *"You look like a man who has been handed a ship by a friend and is waiting to find out what it costs."* A supporting character delivering a polished aphorism about Rook's inner state. Nobody talks like this, and it outsources the banned move of telling the reader what the POV character feels. A doctor takes a pulse and says a number. Give the character something to *do* and a fact to report; let the reader infer the rest. The same line then added *"That is not a medical finding. It is a base rate"* — a character making a statistics pun about his own dialogue, which is the concept being name-dropped as a punchline instead of taught. **A crew member's statistical habit must show up as how they reason about their own job** (Ferrante: "I have cleared nine officers into a first command; seven slept badly for a fortnight and then stopped"), never as them saying the term.
19. **The fiction naming the course's own structure.** "since Act I", "in the last module", "the course teaches". Rook does not live in a numbered Act and has never heard of a module. Refer to the *events*: "since Callisto", "since the shakedown", "the morning he sent the preliminary". Known instances remaining in `content/act-4/09`, `act-5/02`, `act-5/04` and `act-6/01`; clear them when those Acts are revised.
20. **The behaviour gloss: "which from her is X", "the way a man does when Y".** The single most frequent tic in the book — **28 instances** when it was finally counted, and two of them survived a full revision because they read as characterful. They are not. "Ferrier initials the caption and says nothing, *which from her is the answer*." "He says it *the way a man says something he has decided rather than found*." "He says so, and then says it again in different words, *the way people do when they want to be argued with*." Each one performs an action and then tells the reader what it signified, which is the same failure as *You believe her* wearing a subordinate clause. **Cut the gloss.** If the action alone does not carry the meaning, change the action: not "he says it like a man who has decided", but "He has not run it without her." The tells are `which from`, `which is how`, `the way a/one/people`, and any clause after a comma whose job is interpretation rather than event.
21. **The sentence that keeps going after it has landed.** *"It does not yet say why, **and I want that said out loud before anyone writes it down**."* The line was finished at "why". The trailing clause adds an instruction nobody needed, and it weakens the stop. Ferrier's authority is in the full stop, not in the sentence that chases it. This is distinct from the arch trailing clause (#14) — that one is wry, this one is merely flab — and it is the commonest way a good line gets spoiled. **Read every line of dialogue backwards from its end and ask where it could have stopped.** Usually one clause earlier.
22. **Narration that admits it is a lesson.** "You give the table to Ebele, *because he is the one who has to learn it*." The fiction must never acknowledge its own pedagogical purpose. Ebele is an officer doing a job, not a stand-in for the learner.
