...
- **Generating story.**
  ...
  - **2,169 advisories:** `mark ~ U(1, 12)` — the Board's "spread along the Lane"; the mean mark of **all 2,200 records is 7.1 ± 0.1** (tune seed). The mean mark of the 31 losses alone is ≈ 8.5. Each of the 19 diverted hulls also has one *profile-deviation advisory* at Mark 9 (late check-in, +2–3 d).
- **Headline numbers.** Classification of the 31: unknown 22 (19 Uruk, 3 Ceres) · accident 5 (Ceres) · piracy 4 (Uruk). Uruk classified 23, Ceres 8. Time-to-silence: two piles — 12 abrupt (< 1 min), 19 gradual (3–9 h); the mean (≈ 3.4 h) sits in the gap. Mark-at-last-contact: 23 of 31 in [9, 10.5]. Tightest 30-day cluster of loss dates: **5** (2182-02-24 → 03-21: Kestrel Bough, Nicholson Regio, Thessaly Ember, Xibalba Sulcus, Hygiea Promise). Lost hulls' declared cargo value: median below the Lane median. Hull age: no difference.
...
- **Variables.** `hull`; `owner_class`; `drive` (Mk 3 / Tessera-C / other); `declared_mass` (t, departure mass incl. propellant); `fuel_loaded` (t); `examiner`; `accel_mark2` (mgee); `plume_power` (TW); `inferred_mass` (t) = thrust(plume, drive model)/accel; `later_lost` (bool).
- **Generating story.** `declared_mass ~ U(14,000, 27,000)` for standard hulls; **one Mercantile bulk hauler, MV *Cyrene Ore*, at 60,000 t** (the leverage point). Honest: `inferred_mass = declared × (1 + N(0, 0.020))` — **residual SD 2.0% of declared mass** (s ≈ 400 t at 20,000 t); `fuel_loaded = 0.215 × declared × (1 + N(0, 0.010))`. **The 19 diverted:** `inferred = declared × (1 + h)`, `h ~ U(0.06, 0.09)`; `fuel = declared × (1 + h) × (1 − e^(−(242 + Δv_K)/1000))` with Δv_K from DS-16 (≈ 0.243–0.266 of declared; mean 0.257). The 12 other lost hulls are honest.
- **Headline numbers.** r(inferred, declared) ≈ 0.99; LSRL slope ≈ 1.00, intercept ≈ 0; the 19 sit at +6–9% (z ≈ 3–4.5 on the residual scale) and are the only points above +5%; no fan in the residual plot. *Cyrene Ore* has leverage > 4/n and moves r² visibly; with/without changes the slope in the third decimal. Fuel vs declared mass is linear through the origin (slope 0.215); the 19's fuel points sit high (slope 0.257 on the 19 alone). Board's report quotes r² and omits s.
...
- **DS-03b · Lane masters' survey (yours).** Frame = the Lane schedule, **260 masters in transit** at MET 57 (Perrine 88, Mercantile 110, independent 62). Design chosen by the learner: SRS n = 40; or stratified 15/15/15; or cluster by convoy (6 convoys of ~7). Responses generated per stratum: belief in piracy — independents 31%, Mercantile 48%, Perrine (responders) 100%; "seen a Sulcus hull turning late" — 6 of 40 (independents and Mercantile only); Sulcus nonresponse: 9 contacted, 2 answered "no comment," 7 silent. Link-time cost per contact: 55 ± 15 min of Solberg's time; cluster contacts share a link (cost 20 min each after the first).
...
- **DS-04b · *Asgard* escort lottery (yours, MET 74–79).** 12 transits departing Mark 6–7 in one week; 6 escorted, chosen by lot (Solberg's random digits, logged). Response: **off-nominal advisories per transit** (the Register's minor-record count). Escorted: mean 1.5 (values {0,1,1,2,2,3}); unescorted: mean 2.2 ({1,1,2,2,3,4}). Difference −0.7; randomization distribution (all C(12,6) = 924 relabellings) puts the observed difference at p ≈ 0.3. Losses: none (a 1%-per-transit event cannot be seen in 12 transits — that is the lesson).
- **Used by.** act-3-04, 3-05, checkpoint III, 6-06 (power foreshadowing).

### DS-05 · The Chief's tables (sink endurance and subsystem loads)
- **Provenance.** *Nightjar*'s design book (Adlinda) corrected by Sandoval's shakedown runs. Sink capacity **78 GJ** (21.7 MWh); skin loop 70 kW.
- **Cold profiles (design → measured shakedown → post-Act-VII refit).** Quiet 190 kW: 114 h → 108 h → 99 h. **Watch 320 kW: 68 h → 64 h → 59 h.** Standby 670 kW: 32 h → 31 h → 28 h. The design curve is ~5% optimistic (Prologue).
